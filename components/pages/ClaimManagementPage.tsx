...
import { Calendar, MapPin, User as UserIcon, Package, MessageCircle, Phone, Mail, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export function ClaimManagementPage({
  ...
  const myItemsWithClaims = items.filter(
    (item) => item.reportedBy === userEmail && claims.some((claim) => claim.itemId === item.id)
  );
  ...
  const renderClaimCard = (claim: Claim, showActions: boolean = true) => {
    const item = getItemById(claim.itemId);
    if (!item) return null;
  ...
  const renderMyOwnClaimCard = (claim: Claim) => {
    const item = getItemById(claim.itemId);
    if (!item) return null;
