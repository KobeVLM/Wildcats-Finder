...
export function HomePage({ items, onClaim }) {
  ...
  const stats = {
    ...
    lost: items.filter((i) => i.type === "lost" && i.status === "active")
      .length,
    found: items.filter((i) => i.type === "found" && i.status === "active")
      .length,
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <Paper
          ...
          sx={{
            background:
              "linear-gradient(to right, #800020, rgba(128, 0, 32, 0.8))",
            color: "white",
            p: 4,
            borderRadius: 3,
          }}
        >
          <Box sx={{ maxWidth: 768 }}>
            ...
            >
              CIT-U's Lost & Found platform helping students reunite with their
              belongings
            </Typography>
